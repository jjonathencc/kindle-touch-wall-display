...
what to jailbreak, what to install, and the exact failure modes I hit
...
6. [My custom code: the KUAL extension](#my-custom-code-the-kual-extension)
...
built by someone else (see [The server](#the-server)); I point you to it
rather than shipping a copy. The plugin that actually renders a calendar
into an image lives on my own server and isn't included here either, more
on why in [My custom code](#my-custom-code-the-kual-extension).
...
- **A machine on your LAN to run the display server.** Nothing exotic, this runs comfortably on a small home server or even a Raspberry Pi
...
- **A source of calendar data and a plugin to render it,** which you'll
  ...
  [My custom code](#my-custom-code-the-kual-extension).

## Jailbreak (not included here)

**This repo does not host, mirror, link directly to, or explain how to
obtain jailbreak files.** I'm not going to pretend that's neutral, it's
...
your device and firmware version and tells you which jailbreak applies, use that instead of guessing. This device used **SpringBreak**
...
- **[KUAL](https://github.com/KindleTweaks/PEKI)** (Kindle Unified
  ...
  this one. I used the **PEKI** build
...
- **MRPI** (MobileRead Package Installer), used to install KUAL. Get the
  ...
  [post-jailbreak install guide](https://kindlemodding.org/jailbreaking/post-jailbreak/installing-kual-mrpi/kual-mrinstaller-khf.zip), check the MRPI package's own `VERSION` file for which firmware range
...
**What I built on top of, and where the design came from:** the
suspend-to-RAM approach, arm an RTC alarm, suspend the whole device to
RAM between refreshes, wake for the next fetch, is not something I
...
What I built differently: kindle-dash's docs describe installing over
...
I don't distribute FBInk; it's already on the device once you're
...
I didn't write a server from scratch. This runs against a self-hosted fork
of **[usetrmnl/byos_fastapi](https://github.com/usetrmnl/byos_fastapi)**, a FastAPI implementation of the TRMNL "Bring Your Own Server" API, built by
Rui Carmo on earlier work by [@ohAnd](https://github.com/ohAnd/trmnlServer).
**I are not shipping that code here.** It's someone else's open-source
...
## My custom code: the KUAL extension
...
actual PNG this script fetches and draws lives inside my BYOS server fork,
not in this repo. I left it out on purpose, it's wired directly into
...
sanitize properly, and rather than rush a half-scrubbed version out, I'm
...
unattended," full stop. I treat trading that away for reliability, e.g.
...
schedule every time, so the alarm and suspend cycle are fine, but the
radio won't rejoin the access point for hours. Worst case so far was
about 33 hours, and it recovered on its own. Battery is unaffected,
since it keeps suspending normally between retries. What you lose is a
fresh image, not runtime. I've added diagnostic logging for it but
haven't found the cause yet.

**(B) A suspend that doesn't hold can leave the screen stale.**
Sometimes a suspend returns early instead of holding for the full
interval. The script falls back to staying awake rather than risk never
waking, which is the safe choice, but a timing bug in that fallback let
it lose track of real elapsed time and sit idle for nearly two hours
...
instead of drifting, and it reports itself to the server so you can see
it without pulling the device log. What I still don't know is why the
suspend fails in the first place. I've instrumented it to capture what
wakes the device, so the next occurrence should answer that.

Neither bug has caused data loss or bricked anything. Both recover on
their own. If you get impatient, one power button press and KUAL > TRMNL
Calendar > Start calendar display brings it back.
...
- **The BYOS FastAPI server** you'll self-host is its own separate
  ...
  directly; nothing here inherits or modifies it, since I don't ship any
  of that code.
- **FBInk**, which the jailbreak provides and this script shells out to,
  is GPLv3+. I don't distribute FBInk ourselves, it's already present on
